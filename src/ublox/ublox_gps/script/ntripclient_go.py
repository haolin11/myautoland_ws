...
import serial
from ublox_msgs.msg import NavPVT # Copy from ublox_driver
...
## NTRIP configuration
_ntrip_server = 'rtk2go.com'
_ntrip_port = 2101
_ntrip_mountpoint = 'aamakinen'  #Evijarvi, Finland
#_ntrip_mountpoint = 'CN_Sh_VXMT' #Shanghai, China
_ntrip_mountpoint = 'pgzrtcm3' #Hanzhou, China
#_ntrip_mountpoint = 'ENIWA-K-F9P' #Eniwa, Japan
_ntrip_username = ''
_ntrip_password = ''
...
class ntripclient:
    def __init__(self):
        ...
        self.pvt_topic = "/gps/navpvt" # Topic from ublox_driver
        ...
        self.sub_pvt = rospy.Subscriber(self.pvt_topic, NavPVT, self.pvt_callback)
    ...
    def pvt_callback(self, pvt_msg):
        ...
        if self.pvt_valid:
            self.pvt_lat = pvt_msg.lat * 100   # x100 for GxGGA
            self.pvt_lon = pvt_msg.lon * 100  # x100 for GxGGA
            self.pvt_alt = pvt_msg.height
            self.pvt_status = 1
            self.pvt_num_sv = pvt_msg.numSV # Used sv of GPS,GLONASS,GALILEO,Beidou , not only GPS
            if self.pvt_num_sv > 12:    # max = 12 in GPGGA
                self.pvt_num_sv = 12    # can't get used GPS number, but it doesn't matter
            self.pvt_pdop = pvt_msg.pDOP
