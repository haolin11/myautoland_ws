...
UAV_estimator::UAV_estimator(ros::NodeHandle &nh)
{
    ...
    else if (location_source == prometheus_msgs::UAVState::GPS || location_source == prometheus_msgs::UAVState::RTK)
    {
        ...
        // 【订阅】RTK降落点经纬度，来自rtk_data_trans解析节点！！
        RTK_land_global_position_sub = nh.subscribe<sensor_msgs::NavSatFix>("/rtk_land_pos/global_position/global", 1, &UAV_estimator::set_RTK_land_pose_cb, this);
        // 【发布】ENU坐标系下的RTK降落点位置
        RTK_land_local_position_pub = nh.advertise<geometry_msgs::PoseStamped>(uav_name + "/rtk_land_pos", 1);


        // 【订阅】GPS状态，来自飞控
        gps_status_sub = nh.subscribe<mavros_msgs::GPSRAW>("/mavros/gpsstatus/gps1/raw", 10, &UAV_estimator::gps_status_cb, this);
        // 【订阅】无人机当前经纬度，来自飞控
        px4_global_position_sub = nh.subscribe<sensor_msgs::NavSatFix>("/mavros/global_position/global", 1, &UAV_estimator::px4_global_pos_cb, this);
        // 【订阅】无人机当前真实高度，来自飞控
        px4_rel_alt_sub = nh.subscribe<std_msgs::Float64>("/mavros/global_position/rel_alt", 1, &UAV_estimator::px4_global_rel_alt_cb, this);
        // 【订阅】设置ENU坐标系下无人机的位置偏移量  坐标系:ENU系 - 来自地面站/终端窗口
        set_local_pose_offset_sub = nh.subscribe<prometheus_msgs::GPSData>(uav_name + "/prometheus/set_local_offset_pose", 1, &UAV_estimator::set_local_pose_offset_cb, this);
        // 【发布】ENU坐标系下的位置偏移量
        local_pose_offset_pub = nh.advertise<prometheus_msgs::OffsetPose>(uav_name + "/prometheus/offset_pose", 1);
        // 【订阅】GPS卫星数量
        gps_satellites_sub = nh.subscribe<std_msgs::UInt32>("/mavros/global_position/raw/satellites", 1, &UAV_estimator::gps_satellites_cb, this);
    }
...
{
    // std::cout<<"uav_state_update: "<<uav_state_update<<std::endl;
    if (!uav_state_update)
    {
        return;
    }
    ...
    // std::cout<<"state pub! "<<std::endl;
    // std::cout<<"uav_state: "<<int(uav_state.armed) <<std::endl;
...
void UAV_estimator::t265_cb(const nav_msgs::Odometry::ConstPtr &msg)
{
    ...
    // uav_state.position[0] = -(msg->pose.pose.position.y) + offset_pose.y;
    // uav_state.position[1] = msg->pose.pose.position.x + offset_pose.x;
...
int UAV_estimator::check_uav_odom()
{
    ...
    // cout<<"uav_vel_xy.norm(): "<<uav_vel_xy.norm()<<"  "<<"vel_Z: "<<uav_state.velocity[2]<<endl;
...
void UAV_estimator::set_RTK_land_pose_cb(const sensor_msgs::NavSatFix::ConstPtr &msg)
{
    GeographicLib::Geocentric earth(GeographicLib::Constants::WGS84_a(), GeographicLib::Constants::WGS84_f());
    Eigen::Vector3d rtk_land_gps;
    Eigen::Vector3d rtk_land_ecef;
    Eigen::Vector3d current_uav_gps;
    Eigen::Vector3d current_uav_ecef;
    Eigen::Vector3d ecef_offset;
    Eigen::Vector3d enu_offset, enu_offsetUAV;

    rtk_land_gps[0] = msg->latitude;
    rtk_land_gps[1] = msg->longitude;
    rtk_land_gps[2] = msg->altitude;

    current_uav_gps[0] = uav_state.latitude;
    current_uav_gps[1] = uav_state.longitude;
    current_uav_gps[2] = uav_state.altitude;

    earth.Forward(rtk_land_gps[0], rtk_land_gps[1], rtk_land_gps[2], rtk_land_ecef[0], rtk_land_ecef[1], rtk_land_ecef[2]);
    ...
    // {
        ...
        rtk_land_pos.pose.position.z = uav_state.position[2] + enu_offset[2] - 0.2;
        // rtk_land_pos.pose.orientation = uav_state.attitude_q;
    // }

    //TODO: 更换部分代码，采用过渡点的方法，目前先采用直接发送实际降落点
    // // 计算过渡点相对于无人机的位置偏移量
    // Eigen::Vector3d transition_offset;
    // transition_offset[0] = enu_offset[0];
    // transition_offset[1] = enu_offset[1];
    // transition_offset[2] = enu_offset[2] + 2.0; // 上方两米

    // // 计算无人机当前位置与过渡点的直线距离
    // double distance_to_transition = transition_offset.norm();

    // // 根据距离判断是否发送实际降落点
    // if (distance_to_transition < 0.2) { // 小于20厘米，无人机接近过渡点，发送实际降落点
        // if(msg->position_covariance[0]>0.3 || msg->position_covariance[4]>0.3) //判断降落点RTK信号的稳定性
        // {
        //     rtk_land_pos.pose.position.x = uav_state.position[0];
        //     rtk_land_pos.pose.position.y = uav_state.position[1];
    ...
    // } else {
    //     // 无人机远离过渡点，发送过渡点
    //     rtk_land_pos.pose.position.x = uav_state.position[0] + transition_offset[0];
    //     rtk_land_pos.pose.position.y = uav_state.position[1] + transition_offset[1];
    //     rtk_land_pos.pose.position.z = uav_state.position[2] + transition_offset[2];
    // }
    
    RTK_land_local_position_pub.publish(rtk_land_pos);
}
