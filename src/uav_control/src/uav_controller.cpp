...
UAV_controller::UAV_controller(ros::NodeHandle &nh)
{
    ...
    // 【参数】是否使用RTK初始init悬停点
    nh.param<bool>("set_init_RTK_hoverPos", set_init_RTK_hoverPos, false);
    ...
    // 【参数】降落时自动上锁高度
    nh.param<float>("control/Disarm_height", Disarm_height, 0.1);
    // 【参数】降落速度
    ...
    px4_position_target_sub =
        ...
        nh.subscribe<mavros_msgs::PositionTarget>("/mavros/setpoint_raw/target_local",
                                                  1,
                                                  &UAV_controller::px4_pos_target_cb, this);
    ...
    px4_attitude_target_sub =
        ...
        nh.subscribe<mavros_msgs::AttitudeTarget>("/mavros/setpoint_raw/target_attitude",
                                                  1,
                                                  &UAV_controller::px4_att_target_cb, this);
    ...
    if(set_init_RTK_hoverPos)
    {
        //【订阅】订阅RTK降落点ENU坐标系位置（用于刷新RTK降落目标点）
        // rtk_land_local_position_sub = nh.subscribe<geometry_msgs::PoseStamped>("/rtk_land_pos",
        //                   1,
        //                   &UAV_controller::set_rtk_land_pos_cb, this);
        //TODO: 确认话题名称
        rtk_land_local_position_sub = nh.subscribe<geometry_msgs::PoseStamped>("/rtk_land_pos/polyline_target",
                                            1,
                                            &UAV_controller::set_rtk_land_pos_cb, this);
    }
    ...
    px4_setpoint_raw_global_pub = 
        ...
        nh.advertise<mavros_msgs::GlobalPositionTarget>("/mavros/setpoint_raw/global", 1);
    ...
    // uav_command.yaw_ref = 1.57;
    uav_command.yaw_ref = Hover_yaw;

    RTK_des_position << 0.0, 0.0, 0.0;
    RTK_des_yaw = Hover_yaw;
...
void UAV_controller::mainloop()
{
    ...
    case CONTROL_STATE::RC_POS_CONTROL:
        ...
        if(set_init_RTK_hoverPos == true){
            // set_RTK_des_pose();
            pos_des = RTK_des_position;
            vel_des << 0.0, 0.0, 0.0;
            acc_des << 0.0, 0.0, 0.0;
            yaw_des = RTK_des_yaw;
            // cout<<"pos_des = RTK_des_position!!!!!!!!"<<endl;
        }else
        {
    ...
    case CONTROL_STATE::LAND_CONTROL:
        // std::cout<<"into landcontrol"<<std::endl;
        ...
        }else
        {
            ...
            // std::cout<<uav_pos[2]<<std::endl;
            if (uav_pos[2] < Disarm_height)
            {
                //std::cout<<"enable_emergency!!!!!!!!!!!!!"<<std::endl;
                // 进入急停
...
void UAV_controller::set_RTK_des_pose()
{
    ros::Time now = ros::Time::now();
    double delta_t = (now - last_set_hover_pose_time).toSec();
    last_set_hover_pose_time = now;
    ...
    RTK_des_position[0] = 100;
    RTK_des_position[1] = 100;
    RTK_des_position[2] = 100;
    RTK_des_yaw = 100;
...
void UAV_controller::set_command_des()
{
    if (uav_command.Agent_CMD == prometheus_msgs::UAVCommand::Init_Pos_Hover)
    {
        ...
        // yaw_des = 0.0;
        // std::cout<<std::endl;
        // std::cout<<"【Init_Pos_Hover】!!!! yaw_des = "<<yaw_des<<std::endl;
        // std::cout<<std::endl;
          // Init_Pos_Hover 当前位置悬停
        if (uav_command_last.Agent_CMD != prometheus_msgs::UAVCommand::Init_Pos_Hover)
        {
            Hover_position[0] = uav_pos[0] - offset_pose.x;
            Hover_position[1] = uav_pos[1] - offset_pose.y;
            Hover_position[2] = uav_pos[2];
            Hover_yaw = uav_yaw;
            // cout<<WHITE<<"Current_POS_HOVER悬停位置"<<Hover_position[0]<<"  "<<Hover_position[1]<<"  "<<Hover_position[2]<<TAIL<<endl;
...
void UAV_controller::uav_cmd_cb(const prometheus_msgs::UAVCommand::ConstPtr &msg)
{
    if(control_state != CONTROL_STATE::COMMAND_CONTROL)
    {
        ...
        uav_command.Agent_CMD = prometheus_msgs::UAVCommand::Init_Pos_Hover;
        // uav_command.Agent_CMD = prometheus_msgs::UAVCommand::Current_Pos_Hover; //hover in current position!!!
        return;
    }
...
{
    uav_state = *msg;
    // std::cout<<"is armed? "<<uav_state.armed<<std::endl;
...
void UAV_controller::px4_rc_cb(const mavros_msgs::RCIn::ConstPtr &msg)
{
    ...
    static int Timer_hoverPos = 0;
    static ros::Time last_Timer_hoverPos_time = ros::Time::now();
    Timer_hoverPos ++;
    if(Timer_hoverPos > 30 && uav_state.mode != "OFFBOARD" && control_state != CONTROL_STATE::COMMAND_CONTROL)
    {
        ros::Time now = ros::Time::now();
        double delta_t = (now - last_Timer_hoverPos_time).toSec();
        last_Timer_hoverPos_time = now;
        Timer_hoverPos = 0;
        set_hover_pose_with_odom();
        cout << RED << " Change hover position! "<< delta_t << " s/times"<< TAIL << endl;
    }
    
    // std::cout<<rc_input.enter_rc_pos_control<<"    "<<control_state<<std::endl;
    // std::cout<<control_state<<std::endl;
    // 重启PX4飞控，条件: 无人机已上锁
    ...
    static int OFFBOARD_number_count = 1;

    if (OFFBOARD_number_count == 1 && control_state == CONTROL_STATE::RC_POS_CONTROL && uav_state.mode == "OFFBOARD")
    {
        
        OFFBOARD_number_count ++;

        set_hover_pose_with_odom();
        cout<<WHITE<<"set_init_RTK_hoverPos: "<<set_init_RTK_hoverPos<<TAIL<<endl;

        if(set_init_RTK_hoverPos == true)
        {
            uav_command.Agent_CMD = prometheus_msgs::UAVCommand::Init_Pos_Hover;
            cout << WHITE << node_name << " Switch to Init_RTK_Pos_Hover" << TAIL << endl;
        }else
        {
            uav_command.Agent_CMD = prometheus_msgs::UAVCommand::Current_Pos_Hover;
            cout << WHITE << node_name << " Switch to Current_Pos_Hover" << TAIL << endl;
        }
...
int UAV_controller::check_failsafe()
{
        ...
        uav_state.position[2] < uav_geo_fence.z_min || uav_state.position[2] > uav_geo_fence.z_max)
    {
        ...
        cout << RED << "  X= " << uav_state.position[0]  << "  Y= " << uav_state.position[1] << "  Z= " << uav_state.position[2]<< TAIL << endl;
        return 1;
    }
...
void UAV_controller::printf_control_state()
{
    ...
    case CONTROL_STATE::RC_POS_CONTROL:
        if(set_init_RTK_hoverPos == true){
            ...
            cout << GREEN << "RTK_des_Pos [X Y Z] : " << RTK_des_position[0] << " [ m ] " << RTK_des_position[1] << " [ m ] " << RTK_des_position[2] << " [ m ] " << TAIL << endl;
        }else
        {
            cout << GREEN << "CONTROL_STATE: [ RC_POS_CONTROL ] " << TAIL << endl;
...
void UAV_controller::offset_pose_cb(const prometheus_msgs::OffsetPose::ConstPtr &msg)
{
    ...
    {
        offset_pose = *msg;
        cout << WHITE <<"offset_pose" <<offset_pose.x<<" "<<offset_pose.y <<TAIL << endl;
    }else
    {
...
void UAV_controller::set_rtk_land_pos_cb(const geometry_msgs::PoseStamped::ConstPtr &msg)
{
    //TODO: 暂时用RTK_des_position代替 set_RTK_des_pose();
    if (uav_state.location_source == prometheus_msgs::UAVState::GPS || uav_state.location_source == prometheus_msgs::UAVState::RTK)
    {
        // set_RTK_des_pose();
        ...
        //     //打印降落高度
            cout << RED << "Landing des: "<<"x:"<< RTK_des_position[0] <<" "<<"y:"<< RTK_des_position[1] <<" " <<"z:"<< RTK_des_position[2] <<" "<< TAIL << endl;
        //     RTK_des_position[2] = 0.5;
        // }

        RTK_des_yaw = uav_yaw; 
        
        // cout << WHITE << node_name << "设置RTK降落点成功!" <<RTK_des_position[0]<<" "<<RTK_des_position[1]<<" "<<RTK_des_position[2]<< TAIL << endl;
    }else
    {
        cout << RED << node_name << "Worng! No GPS, the param [set_init_RTK_hoverPos] should be false!" << TAIL << endl;
    }
}
